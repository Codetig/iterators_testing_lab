...
before :each do
  ...
  describe "#reject" do
    it "should reject even numbers" do
        expect(@arr.select{|num| num%2 == 0}).to eq([2,4])
      end
  end
  ...
  describe "#detect" do
    it "should detect all numbers divisible by 2 and 4" do
        expect(@arr.detect{|x| x%2==0 && x%4==0}).to eq(4)
      end
  end
  ...
  describe "#none" do
    it "should return true if the number is not in the array " do
      expect(@arr.none?{|x| x==56}).to be(true)
    end
    it "should return false if the number is in the array " do
      expect(@arr.none? {|x| x==2}).to be(false)
    end
  end
...
#HASH METHODS
  ...
  describe "hash method - #delete_if" do
    it "should delete key/value pair for any value equal to 100" do
      expect(@hash.delete_if{|x,y| x==:a && y==100}).to eq({:b => 200})
    end
  end
